...
JSON file. These TAR files contain maximum `Z` instances each. For example, a JSON
with 2500 instances and Z = 1000 would make three TAR files:

    {output_prefix}_00000000.tar : instance ID [   0 to  999] from JSON
    {output_prefix}_00000001.tar : instance ID [1000 to 1999] from JSON
    {output_prefix}_00000002.tar : instance ID [2000 to 2499] from JSON
"""
import argparse
import json
import tarfile
...
parser.add_argument(
    "--annotations", help="Path to JSON containing RedCaps annotations."
)
parser.add_argument(
...
parser.add_argument("--output-prefix", help="Path prefix for saving TAR files.")
parser.add_argument(
...
def main(_A: argparse.Namespace):
    ...
    ANNOTATIONS = json.loads(Path(_A.annotations).read_text())["annotations"]
    ...
    tar_handle = tarfile.open(f"{_A.output_prefix}_{SHARD_INDEX}.tar", "w")
    ...
    for ann in ANNOTATIONS:
        ...
        if ADDED_ANNS % _A.shard_size == 0 and ADDED_ANNS > 0:
            tar_handle.close()
            print(f"Saved shard: {_A.output_prefix}_{SHARD_INDEX}.tar")
